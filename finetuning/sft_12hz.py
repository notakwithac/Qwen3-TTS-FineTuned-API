...
def train_programmatic(
    config: dict,
    on_progress=None,
):
    """Run training programmatically (no argparse).
    ...
    # Build a fake argparse Namespace from the config dict
    defaults = {
    ...
    # --- The rest is the same as train() but uses args directly and calls on_progress ---
    global target_speaker_embedding
    target_speaker_embedding = None
    ...
    if args.resume:
        latest_ckpt, latest_epoch = _find_latest_checkpoint_static(args.output_model_path)
        if latest_ckpt is not None:
            MODEL_PATH = str(latest_ckpt)
            start_epoch = latest_epoch + 1
            resume_checkpoint = str(latest_ckpt)
    ...
    # Return the path to the last checkpoint
    return os.path.join(args.output_model_path, f"checkpoint-epoch-{num_epochs - 1}")
