...
def _validate_and_crop_audio_sync(
    ...
    char_name: str,
) -> list[tuple[str, bytes, str, bool, str]]:
    import torch
    import whisperx
    ...
    try:
        ...
        model = _get_whisper_model("large-v3", device=device, compute_type=compute_type)
        ...
        for clip_idx, (filename, wav_bytes, prompt_id) in enumerate(audio_items, 1):
            ...
            try:
                ...
                if not speaker_durations:
                    results.append((filename, wav_bytes, prompt_id, False, ""))
                    continue
                ...
                if len(cropped) > 0 and transcript:
                    ...
                    results.append((filename, out_buf.getvalue(), prompt_id, True, transcript))
                    log.info(
        ...
        return [(filename, wav_bytes, prompt_id, False, "") for filename, wav_bytes, prompt_id in audio_items]
...
async def prepare_dataset_items(
    ...
    *,
    ...
    for filename, wav_bytes, prompt_id, success, transcript in validated_segments:
    ...
    if not final_train_segments:
        raise RuntimeError("No valid segments available for finetuning dataset.")
