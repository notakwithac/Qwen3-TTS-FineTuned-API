...
DATASET_SAMPLE_MIN_SEC = 3.0
DATASET_SHORT_CLIP_MAX_SEC = 5.0   # clips below this are flagged as "short" in manifest
DATASET_CLIP_FLUSH_SEC    = 6.0    # don't flush at sentence boundary until >= 6s accumulated
...
def _normalize_audio_for_dataset_sync(audio_bytes: bytes) -> bytes:
    """
    Normalise loudness and resample.  Silence trimming is intentionally light:
    TTS fine-tuning benefits from natural leading/trailing breath — aggressive
    trimming teaches the model to start/stop unnaturally.
    """
    filters = [
        # Only clip dead silence (>500ms of near-digital-silence) at edges.
        # threshold raised to -55dB (was -45dB) and durations lengthened so
        # soft breath / room tone is preserved.
        "silenceremove=start_periods=1:start_duration=0.5:start_threshold=-55dB",
        "silenceremove=stop_periods=1:stop_duration=0.5:stop_threshold=-55dB",
...
def _chunk_segments(
    ...
    *,
    ...
    for seg in speaker_segments:
        ...
        # Natural flush: sentence boundary AND we've accumulated at least 6s
        if current_has_sentence_end and current_duration >= DATASET_CLIP_FLUSH_SEC:
            chunks.append(current)
            current       = [seg]
            current_start = seg["start"]
            current_end   = seg["end"]
...
def _build_split_results(
    ...
    *,
    ...
    for idx, chunk in enumerate(chunks):
        ...
        results.append((clip_name, normalized_wav, prompt_id, True, asr_transcript, None, is_short))
...
async def prepare_dataset_items(
    ...
    *,
    ...
    for filename, wav_bytes, prompt_id, success, transcript, failure_reason, *extra in validated_segments:
        ...
        # For sub-split clips (filename contains __seg_ or __part_), the original
        # canonical text covers the whole source clip — don't stamp it on a fragment.
